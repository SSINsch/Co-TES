...
class DevignTrainer:
    ...
    def train(self, n_epoch, save=False):
        ...
        total_loss_1, total_acc_1 = 0, 0
        total_loss_2, total_acc_2 = 0, 0
        for step, (_x_func_token, _padded_token_idx_matrix, _y_vuln, ind) in enumerate(self.train_loader):
            # to cpu / to gpu
            ...
            # to model 1
            logits_1 = self.model1(_padded_token_idx_matrix)
            max_predictions, argmax_predictions = logits_1.max(1)
            acc_1 = basic_accuracy(argmax_predictions, _y_vuln)
            acc_1 = to_np(acc_1)
            total_acc_1 += acc_1

            # to model 2
            logits_2 = self.model2(_padded_token_idx_matrix)
            max_predictions, argmax_predictions = logits_2.max(1)
            acc_2 = basic_accuracy(argmax_predictions, _y_vuln)
            acc_2 = to_np(acc_2)
            total_acc_2 += acc_2

            # get loss
            if n_epoch < self.init_epoch:
                loss_1, loss_2, _, _ = loss_coteaching(logits_1, logits_2, _y_vuln,
                                                       self.rate_schedule[n_epoch], ind,
                                                       self.noise_or_not)
            else:
                if self.model_type == 'coteaching_plus':
                    loss_1, loss_2, _, _ = loss_coteaching_plus(logits_1, logits_2, _y_vuln,
                                                                self.rate_schedule[n_epoch], ind,
                                                                self.noise_or_not, n_epoch * step)
                else:
                    raise Exception('Init Epoch is something wrong...')
    ...
    def evaluate(self, model_summary_path, n_epoch, mode='test', load=False):
        ...
        with torch.no_grad():
            for step, (_x_func_token, _padded_token_idx_matrix, _y_vuln, ind) in enumerate(loader):
                # to cpu/gpu
                ind = ind.cpu().numpy().transpose()
                _padded_token_idx_matrix = _padded_token_idx_matrix.to(self.device)
                _y_vuln = _y_vuln.to(self.device)
